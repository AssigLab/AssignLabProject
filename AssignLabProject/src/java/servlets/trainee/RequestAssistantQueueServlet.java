 ...
 * @author rania
 */
public class RequestAssistantQueueServlet extends HttpServlet {
    ...
    public int insertTraineeInAssistantQueue(User user, Lab lab, String comment) {
       // QueueDAO qd=new QueueDAO();
        ...
        int check = queueInt.insertTraineeInAssistantQueue(user, lab, comment);
        return check;
    }
...
//        lab.setIdLab(1);
        ...
        if ((Integer) session.getAttribute("labfound") == 1) {
            ...
            flag = insertTraineeInAssistantQueue(user, lab, comment);
            ...
            if (flag == 2) {
                ...
                request.setAttribute("cantRequestAssistant", "you already in queue");
                RequestDispatcher requestdispatcher = request.getRequestDispatcher("QueueServlet");
                System.out.println("done");
                out.println(" request dao ");
                requestdispatcher.forward(request, response);
            } else if (flag == 1) {
                request.setAttribute("cantRequestAssistant", "Sorry lab is closed");
                RequestDispatcher requestdispatcher = request.getRequestDispatcher("QueueServlet");
                System.out.println("done");
                out.println(" request dao ");
                requestdispatcher.forward(request, response);
